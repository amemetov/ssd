...
def randomize_contrast(img, contrast_lower=0.5, contrast_upper=1.5):
...
def randomize_hue(img, hue_delta=18):
    hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
...
def randomize_saturation(img, saturation_lower=0.5, saturation_upper=1.5):
    hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
