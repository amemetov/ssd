...
from .prior_box import doCreatePriorBoxes, default_prior_variance

# generated by prior_box.gen_prior_boxes_config method
vgg16_layers_config = [
    {'layer': 'conv4_3', 'normalization': 20,
     'layer_width': 38, 'layer_height': 38, 'min_size': 30.0, 'max_size': 60.0, 'aspect_ratios': [1.0, 1.0, 2.0, 0.5]},

    {'layer': 'fc7', 'normalization': -1,
     'layer_width': 19, 'layer_height': 19, 'min_size': 60.0, 'max_size': 111.0, 'aspect_ratios': [1.0, 1.0, 2.0, 0.5, 3, 1/3.0]},

    {'layer': 'conv6_2', 'normalization': -1,
     'layer_width': 10, 'layer_height': 10, 'min_size': 111.0, 'max_size': 162.0, 'aspect_ratios': [1.0, 1.0, 2.0, 0.5, 3, 1/3.0]},

    {'layer': 'conv7_2', 'normalization': -1,
     'layer_width': 5, 'layer_height': 5, 'min_size': 162.0, 'max_size': 213.0, 'aspect_ratios': [1.0, 1.0, 2.0, 0.5, 3, 1/3.0]},

    {'layer': 'conv8_2', 'normalization': -1,
     'layer_width': 3, 'layer_height': 3, 'min_size': 213.0, 'max_size': 264.0, 'aspect_ratios': [1.0, 1.0, 2.0, 0.5]},

    {'layer': 'conv9_2', 'normalization': -1,
     'layer_width': 1, 'layer_height': 1, 'min_size': 264.0, 'max_size': 315.0, 'aspect_ratios': [1.0, 1.0, 2.0, 0.5]}
]

def gen_vgg16_feature_layers_config(img_w, img_h, variance=default_prior_variance, clip=False, offset=0.5):
    return _gen_feature_layers_config(vgg16_layers_config, img_w, img_h, variance=variance, clip=clip, offset=offset)

def _gen_feature_layers_config(layers_config, img_w, img_h, variance=default_prior_variance, clip=False, offset=0.5):
    ...
    for layer_config in layers_config:
        ...
        # add to both lists
        feature_layers_config.append({'layer': layer, 'normalization': normalization,
                               'num_prior': len(aspect_ratios), 'pbs': prior_boxes})
        flattened_pbs.append(prior_boxes)

    flattened_pbs = np.concatenate(flattened_pbs, axis=0)
    return feature_layers_config, flattened_pbs
...
def CreateMultiBoxHead(net, num_classes, layers_config,
